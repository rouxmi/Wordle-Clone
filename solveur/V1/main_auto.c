...
char* coloration(const char * mot_a_test,const char * mot_a_dev,int size){
	char* copy_test=malloc(sizeof(char)*(size+2));
	char* copy_dev=malloc(sizeof(char)*(size+2));
...
void solvauto() {
	...
	for (int g=0;g<2;g++){
		for (int t=2;t<20;t++){
			...
			rewind(ptr);
			element E;
			element  *T=malloc(sizeof(E)*(tailledict+1)*(300));
			if (NULL == ptr) {
			...
			for (int j=0;j<tailledict;j++){
				...
				for (int i=0;i<nb_essais;i++){
					...
					ajout_mot(word_test,i,&T[j],color,len_mot,nb_essais);
					free(color);
				}
